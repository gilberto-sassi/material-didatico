...
\begin{frame}{Nomenclatura}
 \begin{description}
  ...
  \item[Estatística] \textit{Cálculo} (função) da amostra;
  \vfill
  
%  \item[Estimador] Estatística escolhida para se aproximar um parâmetro (\textit{antes de se realizar a conta});
%  \vfill
%  
  \item[Estimativa] Característica de amostra obtida usando uma estatística; 
 \end{description}
\end{frame}

\begin{frame}{Exemplo}
 Intenção de voto para Lula em outubro de 2026.
 
 \begin{description}
  \item[População] Todos eleitores aptos a votar em outubro de 2026;
  \vfill

  \item[Amostra] Três mil eleitores que serão questionados se pretendem, ou não, em Lula;
	\vfill
...
\begin{block}{Exemplo -- Tabela 2.1 do livro do Morettin}
	...
	\begin{table}
		...
		\begin{tabular}{ll}
			...
			Região da procedência &  Qualitativa Nominal\\ \bottomrule[0.05cm]
		\end{tabular}
	\end{table}
\end{block} 
\end{frame}
...
\begin{frame}{Exemplo}
	...
	\begin{table}
		...
		\begin{tabular}{l|ccc}
			\toprule[0.05cm]
			Escolaridade  & Frequência & Frequência Relativa & Porcentagem\\ \midrule[0.05cm]
...
\begin{frame}{Exemplo}
	...
	\begin{table}
		...
		\begin{tabular}{l|ccc}
			\toprule[0.05cm]
			Número de filhos & Frequência & Frequência relativa & Porcentagem\\ \midrule[0.05cm]
			$0$ & $20$ & $0,5556$ & $55,56\%$\\
			$1$ & $5$ & $0,1386$ & $13,86\%$\\
			$2$ & $7$ & $0,1944$ & $19,44\%$\\
			$3$ & $3$ & $0,0833$ & $8,33\%$\\
...
\begin{frame}{Exemplo}
	...
	\begin{table}
		...
		\begin{tabular}{l|ccc}
			\toprule[0.05cm]
			Salário & Frequência & Frequência relativa & Porcentagem\\ \midrule[0.05cm]
			$[4,8)$ & $10$ & $0,2778$ & $27,78\%$\\
