  ...
  \begin{enumerate}[1)]
   ...
   \item \begin{align*}
          md(x) = \begin{cases}
                  ...
                  \dfrac{ x_{\left( \left\lfloor \frac{n+1}{2} \right\rfloor \right)}+x_{\left( \left\lceil \frac{n+1}{2} \right\rceil \right)} }{2}, & \mbox{se } n \mbox{ é par},\\
                  \end{cases}
         \end{align*}
...
{\small
  ...
  \begin{align*}
   md(x) &= \dfrac{ x_{\left( \left\lfloor \frac{4+1}{2} \right\rfloor \right)} + x_{\left( \left\lceil \frac{4+1}{2} \right\rceil \right)} }{2} = \dfrac{x_{(2)} + x_{(3)} }{2}=\dfrac{2+3}{2} = 2,5.
  \end{align*}
  \end{block}
...
Considere a variável Número de Filhos com tabela de distribuição de frequência dada por
 ...
 \begin{align*}
  ...
  &x_{(36)} = 5
 \end{align*}
 O tamanho amostral $n=36$ é par, então $md(x) = \dfrac{ x_{\left( \left\lfloor \frac{36+1}{2} \right\rfloor \right)}+x_{\left( \left\lceil \frac{36+1}{2} \right\rceil \right)}}{2} = \dfrac{ x_{(18)}+x_{(19)}}{2} = \dfrac{0+0}{2}=0$.
}
\end{frame}

\begin{frame}{Uso da tabela de distribuição de frequência: caso contínuo}
...
\begin{frame}{Solução exemplo}
 ...
 \begin{align*}
  md(s)&=\dfrac{ s_{\left( \left\lfloor \frac{36+1}{2} \right\rfloor \right)}+s_{\left( \left\lceil \frac{36+1}{2} \right\rceil \right)}}{2}\\
  &=\dfrac{ s_{(18)}+s_{(19)}}{2}\\
  &= \dfrac{10+10}{2}\\
  &=10.
 \end{align*}
...
\begin{frame}{Solução -- exemplo.}
 \begin{enumerate}[a)]
  ...
  \begin{align*}
   md(x) =\dfrac{ x_{\left( \left\lfloor \frac{50+1}{2} \right\rfloor \right)}+x_{\left( \left\lceil \frac{50+1}{2} \right\rceil \right)}}{2} =\dfrac{ x_{(25)}+x_{(26)}}{2} = \dfrac{0+1}{2} = 0,5.
  \end{align*}
...
\begin{frame}{Assimetria}
	\begin{figure}
		\centering
		\includegraphics[height=8cm]{boxplot_assimetria.png}
	\end{figure}
\end{frame}

\begin{frame}{Assimetria}
Podemos introduzir uma medida numérica de assimetria, denominado coeficiente de Bowley:
\begin{align*}
...
Construa o diagrama de caixa e calcule o coeficiente de Bowley.
  
  {\small
\begin{table}[h]
	\centering
	\caption{População das Cidades Brasileiras (em milhões)}
	\begin{tabular}{l|lc}
		\toprule
		\textbf{UF} & \textbf{Cidade} & {\textbf{População (em milhões de habitantes)}} \\
		\midrule
		SP & São Paulo      & 11,45 \\
		RJ & Rio de Janeiro  & 6,21  \\
		DF & Brasília        & 2,82  \\
		CE & Fortaleza       & 2,43  \\
		BA & Salvador        & 2,42  \\
		MG & Belo Horizonte  & 2,32  \\
		AM & Manaus          & 2,06  \\
		PR & Curitiba        & 1,77  \\
		PE & Recife          & 1,49  \\
		GO & Goiânia         & 1,44  \\
		RS & Porto Alegre    & 1,33  \\
		PA & Belém           & 1,30  \\
		SP & Guarulhos       & 1,29  \\
		SP & Campinas        & 1,14  \\
		MA & São Luís        & 1,04  \\
...
\begin{frame}{Exemplo}
	\begin{enumerate}
		\item Primeiro quartil: $(n+1)\cdot 0,25 = (15+1)\cdot 0,25=4$ e $x_{(4)}=1,30$;
		\vfill 
		
		\item Segundo quartil: $(n+1)\cdot 0,5 = (15+1)\cdot 0,5=8$ e $x_{(8)}=1,77$;
		\vfill
		
		\item Terceiro quartil: $(n+1)\cdot 0,75 = (15+1)\cdot 0,75=12$ e $x_{(12)}=2,43$;
		\vfill
		
		\item Intervalo interquartílico: $dq=q_3 - q_1 = 2,43 - 1,30=1,13$;
		\vfill
		
		\item $LI = q_1 - 1,5 \cdot dq = 1,30 - 1,5 \cdot 1,13 = -0,395$;
		\vfill
		
		\item $LS = q_3 + 1,5 \cdot dq = 2,43 + 1,5 \cdot 1,13 = 5,06$;
		\vfill
		
		\item Pontos exteriores: 11,45 (São Paulo); 6,21 (Rio de Janeiro);
		\vfill
		
		\item Coeficiente de Bowley: $0,17$.
	\end{enumerate}
\end{frame}
...
\begin{frame}{Exemplo}
	\begin{figure}
	\centering
	\includegraphics[height=8cm]{grandes_cidades.png}
\end{figure}	
\end{frame}
