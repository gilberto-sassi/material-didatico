...
\small
 ...
 \begin{table}[ht]
  ...
  \toprule[0.05cm]
  \multicolumn{2}{|c}{}& \multicolumn{2}{c|}{Decisão}\\ \cmidrule{3-4}
  \multicolumn{2}{|c}{} & $H_0$ & $H_1$ (Negação de $H_0$)\\ \cmidrule{3-4}
  \multirow{2}{*}{Situação na população} &$H_0$ & Sem erro (verdadeiro negativo) & Erro tipo I (Falso positivo) \\
  &$H_1$ (Negação de $H_0$) & Erro tipo II (Falso negativo) & Sem erro (Verdadeiro positivo)\\
  \bottomrule[0.05cm]
  \end{tabular}
	}
 \end{table}
...
\normalsize
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{c|c|c|l|l}
			...
			Normal & Não & $\mu \neq \mu_0$ & $RC = \left\{ t_0 \mid t_0 < t_{\frac{\alpha}{2};n-1} \mbox{ ou } t_0 > t_{1-\frac{\alpha}{2}; n-1} \right\}$ & $2\left[ 1 - P\left(t_{n-1} \leq \lvert  t_0 \rvert \right) \right]$ \\ \midrule[0.025cm]
