...
ajuste <- ht_2pop_mean(amostra1, amostra2, var_equal = F, sig_level = g)
sol <- c(
    ajuste$statistic,
    ajuste$p_value
) |> round(2)
...
Suponha que uma pesquisadora precisa decidir entre as hipóteses H~0~: &mu;~1~ - &mu;~2~ = 0  contra H~1~: &mu;~1~ - &mu;~2~ &#8800; 0, e para isso ela coletou uma amostra que está armazenada no arquivo [dados.xlsx](dados.xlsx). Assuma que as variâncias das duas populações são diferentes.  Ao nível de significância &alpha; = `r f(g)`, temos que:
