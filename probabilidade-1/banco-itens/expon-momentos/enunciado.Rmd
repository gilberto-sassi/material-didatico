...
sol <- c(
    "LE" = pexp(a, 1 / media),
    "desigualdade" = pexp(ce, 1 / media) - pexp(b, 1 / media),
    "GE" = 1 - pexp(d, 1 / media),
    "media" = media,
    "segunda_momento_fatorial" = (2 - taxa) / taxa^2,
    "s2" = media^2
)
...
opcoes_caracteristica <- c(
    opcoes["exp_caracteristica"],
    rep("", length(opcoes) - 1)
)
sol_caracteristica <- names(opcoes) |>
    map_chr(~ ifelse(.x == "exp_caracteristica", "1", "0")) |>
    paste0(collapse = "")
# fatorial
opcoes_fatorial <- c(
    opcoes["exp_fatorial"],
...
Question
========
Seja X uma variável aleatória contínua com distribuição Exponencial com tempo médio igual a `r f(media)` . Determine:
...
i. Função geradora de momentos: `r opcoes["exp_momento"]`.
i. Função característica: `r opcoes["exp_caracteristica"]`.
i. Função geradora de momentos fatoriais: `r opcoes["exp_fatorial"]`.
